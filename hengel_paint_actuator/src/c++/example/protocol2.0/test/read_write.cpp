...
// Default setting
#define DXL_ID_10                       10                   // Dynamixel ID: 1
#define DXL_ID_13                       13
...
int main()
{
  ...
  int dxl_goal_velocity = DXL_MAXIMUM_VELOCITY;
  ...
  while(1)
  {
    ...
    // Write goal position
    dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, DXL_ID_13, ADDR_PRO_GOAL_POSITION, dxl_goal_position[index], &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
      printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
    }
    ...
    do
    {
      ...
      printf("[ID:%03d] GoalPos:%03d  PresPos:%03d\n", DXL_ID_13, dxl_goal_position[index], dxl_present_position);
