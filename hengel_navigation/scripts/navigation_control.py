...
class NavigationControl():
    ...
    def runOffset(self):
        ...
        self.pubDelta1=0         #previously published delta_1
        self.pubDelta2=0         #previously published delta_2
        pubIter=0
        ...
        try:
            ...
            if self.cnt_waypoints < 150 and flag_stiff_change and self.next_letter_index != -1:
                ...
                for iteration in range(int(pubIter)):
                    print("---------ITERATION(%d/%d)--------- " % (iteration, pubIter))
                    self.endPoint.x=self.point.x-self.D*cos(self.heading.data)
                    self.endPoint.y=self.point.y-self.D*sin(self.heading.data)
    ...
    def calculate_optimal_D(self, _delX, _delY, _th, _prev_D, _prev_delOmega1, _prev_delOmega2):
        ...
        f_low = self.cost_function(0.21, _a, _b, _b-_prev_D, _prev_delOmega1, _prev_delOmega2)
        f_high = self.cost_function(0.29, _a, _b, _b-_prev_D, _prev_delOmega1, _prev_delOmega2)
        if f_x3 <= f_low and f_x3 <= f_high:
            if x3 >= 0.21 and x3 <= 0.29:
                return self.bound_function(0.0023, x3, _prev_D)
            else:
                if f_low <= f_high:
                    return self.bound_function(0.0023, 0.21, _prev_D)
                else:
                    return self.bound_function(0.0023, 0.29, _prev_D)
        else:
            if f_low <= f_high:
                return self.bound_function(0.0023, 0.21, _prev_D)
            else:
