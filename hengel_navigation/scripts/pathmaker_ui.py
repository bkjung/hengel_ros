...
class PathMaker():
    def __init__(self):
        ...
        self.button_draw = Button(
            ...
            text="Draw",
            relief="raised",
            command=self.command_drawing)
        self.button_draw.grid(row=0, column=0)

        self.button_save = Button(
            self.frame, text="Save", command=self.command_save)
        self.button_save.grid(row=0, column=1)

        self.button_quit = Button(
            self.frame, text="Quit", command=self.command_quit)
    ...
    def callback_move_with_button_held_down(self, event):
        if self.isDrawmode:
            circle = self.c.create_oval(
                ...
                fill='black')
            self.path_append(
                float(event.x) / length_side,
                float(event.y) / length_side)
    ...
    def command_save(self):
        self.file_path = package_base_path + "/hengel_path_manager/path_made/" + time.strftime(
            "%y%m%d_%H%M%S") + ".txt"
