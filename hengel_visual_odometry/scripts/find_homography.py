...
def find_homography(_image, mtx, dist, board_w, board_h, board_sz):
    ...
    pi = math.pi
    theta = math.atan2((corners[0][0][1] -
                        corners[(board_h - 1) * board_w + board_w - 1][0][1]),
                       -(corners[0][0][0] -
                         corners[(board_h - 1) * board_w + board_w - 1][0][0]))
    print(-(corners[0][0][0] -
            corners[(board_h - 1) * board_w + board_w - 1][0][0]))
    print(corners[0][0][1] -
          corners[(board_h - 1) * board_w + board_w - 1][0][1])
    ...
    cv2.circle(
        _image, tuple(imgPts_int[0]), 9,
        (0, 0,
         255))  #img, center, radius, color, thickness=1, lineType=8, shift=0
