...
os.system("mkdir -p " + package_base_path +
          "/hengel_visual_odometry/output_path")
...
class VisualOdometry():
    ...
    def __init__(self):
        ...
        ################
        #Create publisher
        self.vo_point_pub = rospy.Publisher(
            "vo_current_point", Point, queue_size=50)
        init_status = True
        ...
        print(sys.argv[1])
        cv_file_homo = cv2.FileStorage(
            package_base_path + "/hengel_visual_odometry/calibrate_info/" +
            sys.argv[1] + ".xml", cv2.FILE_STORAGE_READ)
        ...
        while not rospy.is_shutdown():
            try:
                ...
                new_corners, st, err = cv2.calcOpticalFlowPyrLK(
                    old_gray, frame_gray, old_corners, None, **lk_params)
        ...
        cv_file = cv2.FileStorage(
            package_base_path + "/hengel_visual_odometry/output_path/" +
            time.strftime("%y%m%d_%H%M%S") + ".xml", cv2.FILE_STORAGE_WRITE)
