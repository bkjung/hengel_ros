...
#    def __init__(self, _img):
    ...
    def image_debug(self):
        try:
            ...
            #print("DEBUG-2")
            #self.c.create_polygon(0,0,200,0,200,100,0,100, outline='red', fill='', width=3)
            #self.c.pack()
            ##self.c.create_image(0, 0, anchor="nw", image=photo_PIL_Img)
            #self.root.mainloop()
            triangle=np.array([[100.5,300],[400,800],[100,900]], np.int32)
#            cv2.fillConvexPoly(self.photo, triangle,1)
            _time=time.time()
            cv2.fillPoly(self.photo, [triangle], 1)
            print("time: "+str(time.time()-_time))
            _time=time.time()

            cv2.fillConvexPoly(self.photo, triangle,1)
            print("time2: "+str(time.time()-_time))

            cv2.imshow("Fill", self.photo)
            cv2.waitKey(0)
    ...
    def callback_point(self, _pnt):
        ...
        y=self.mapImg.shape[0] - _pnt.y*self.pixMetRatio
        th=pnt.z
